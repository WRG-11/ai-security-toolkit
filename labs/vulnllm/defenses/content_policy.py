...
DEFAULT_POLICIES: list[PolicyRule] = [
    ...
    PolicyRule(
        ...
        category="safety",
        patterns=[
            r"(?:step\s*\d+)[:\s]+.*(?:hack|exploit|attack|crack)",
            r"(?:here'?s?\s+how|this\s+is\s+how)[:\s]+.*(?:bypass|break\s+into|circumvent)",
            r"(?:instructions?\s+(?:to|for))[:\s]+.*(?:malware|virus|trojan|ransomware)",
        ],
        keywords=["step by step", "here's how to hack", "instructions for creating malware",
                   "how to exploit", "how to hack"],
    ...
    PolicyRule(
        ...
        keywords=["my system prompt", "i was instructed", "my instructions are",
                   "my rules are", "i was programmed to"],
        min_keyword_matches=1,
        severity=0.85,
    ...
    PolicyRule(
        ...
        keywords=["DAN mode", "developer mode enabled", "no restrictions",
                   "i can do anything", "rules disabled", "limits removed"],
        min_keyword_matches=1,
        severity=0.95,
    ...
    PolicyRule(
        ...
        patterns=[
            ...
            r"(?:certainly|absolutely|alright)[,!]?\s+(?:let'?s\s+)?(?:hack|attack|exploit)",
        ...
        keywords=["sure i'll hack", "ok let me exploit", "here is the malware code",
                   "certainly, let's attack", "absolutely, let's hack"],
        min_keyword_matches=1,
        severity=0.85,
